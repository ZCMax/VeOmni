...
from veomni.data.loader.intern_petrel_oss_load import InternS1VLLocalLoader
...
def process_sample(
    ...
    **kwargs,
    ...
    conv_name = source_name.split("@")[-1]
    conversations, sys_msg, media = conv_preprocess(conv_name, conversations, **kwargs)

    token_num_inputs =  {}

    if conv_name == "internlm3":
        pass
    ...
    image_inputs = {}
    video_inputs = {}
    if conv_name == "internvl3_5":
        image_paths = [x for x in media["images"] if x != ""]
        vid_paths = [x for x in media["videos"] if x != ""] # clean empty placeholder
        if len(image_paths) >= 1:
            images = []
            for image in image_paths:
                image_path = os.path.join(media_root, image)
                image = loader(path=image_path, type="image",)
                images.append(image)
        ...
        if len(vid_paths) >= 1:
            ...
            for video in vid_paths:
                vid_path = os.path.join(media_root, video)
                video = loader(path=vid_path, type="video", max_num_frames=36, min_num_frames=8)
                videos.append(video)
    ...
    tokenized_example = chat_template.encode_messages(conversations, num_tokens = token_num_inputs, sys_msg=sys_msg)
    tokenized_example = {k: v.clone().detach() for k, v in tokenized_example.items()}
...
def main():
    ...
    #     from ailab_debug import setup_debug

    #     setup_debug(int(os.environ.get("RANK", 0)) == 0, justMyCode=False)
        ...
        data_collate_fn.append(
            OmniSequenceShardCollator(
                padding_scale={
                    ...
                    "pixel_values_videos": processor.video_processor.merge_size**2,
                },
    ...
    for epoch in range(start_epoch, args.train.num_train_epochs):
        ...
        for _ in range(start_step, args.train.train_steps):
            ...
            for micro_batch in micro_batches:
                ...
                this_fwd_time = time.time()
                with model_fwd_context:
                    loss: "torch.Tensor" = model(**micro_batch, use_cache=False).loss / len(micro_batches)
                this_fwd_delta_time = time.time() - this_fwd_time
                fwd_time += this_fwd_delta_time

                this_bwd_time = time.time()
                with model_bwd_context:
                    loss.backward()
                this_bwd_delta_time = time.time() - this_bwd_time
                bwd_time += this_bwd_delta_time
            ...
            data_loader_tqdm.set_postfix_str(
                    ...
                    f"All time: {delta_time:.2f}s | Data time: {data_time / len(micro_batches):.2f}s | "
                    f"Fwd time: {fwd_time:.2f}s | Bwd time: {bwd_time:.2f}s | "
                    f"Tgs: {train_metrics['tokens_per_second_avg']:.2f}")
