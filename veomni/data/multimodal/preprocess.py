...
@PREPROCESSOR_REGISTRY.register("internvl3_5")
def internvl3_5_sft_preprocess(conversations, generation_ratio=0.0, **kwargs):
    ...
    if conversations[0]["from"] == "system":
        sys_msg = conversations[0]
        # constructed_conversation.append(["system", ("text", sys_msg["value"])])
        sys_msg = {"role": "system", "content": conversations[0]["value"], "loss_mask": 0,}
        conversations = conversations[1:]
    ...
    for message in conversations:
        ...
        videos = message.get("videos", None)

        message_list = [role]
        message_list.append([("text", value)])
        if images is not None:
            message_list.append([("image", images)])
            media["images"].extend(images if isinstance(images, list) else [images])
        if videos is not None:
            message_list.append([("video", videos)])
            media["videos"].extend(videos if isinstance(videos, list) else [videos])
        
        constructed_conversation.append(message_list)
    return constructed_conversation, sys_msg, media


@PREPROCESSOR_REGISTRY.register("internlm3")
def internlm3_instruction_preprocess(conversations, generation_ratio=0.0, **kwargs):
    constructed_conversation = []
    sys_msg = None
    if conversations[0]["from"] == "system":
        sys_msg = conversations[0]
    ...
    for message in conversations:
        ...
        constructed_conversation.append(message_list)
    return constructed_conversation, sys_msg, None