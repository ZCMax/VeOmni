...
def debug_token_alignment(tokenizer, tokenized_example, max_len=256):
    """
    打印 input_ids 与 labels 的对应关系：
    - token 字符
    - token id
    - 是否参与 loss
    """
    ...
    n = min(len(input_ids), max_len)

    print("=" * 80)
    print("Token Debug Info")
    print("=" * 80)
    print(f"{'idx':>4} | {'input_id':>8} | {'label':>8} | {'char':<25} | loss?")
    print("-" * 80)
    ...
    for i in range(n):
        ...
        # decode 单个 token：需要用 tokenize/de-tokenize 而不是 decode，否则会合并
        token_str = tokenizer.convert_ids_to_tokens([tid])[0]

        needs_loss = (lbl != -100)

        print(f"{i:4d} | {tid:8d} | {lbl:8d} | {token_str:<25} | {needs_loss}")
...
class Qwen2VLChatTemplateWithInternData(Qwen2VLTemplate):
    system_prompt = "You are a helpful assistant."
    def _get_system_mesage(self):
        system_message = {
            "role": "system",
    ...
    def encode_messages(
        ...
        sys_msg = kwargs.get("sys_msg", None)
        if sys_msg is None:
            sys_msg = self._get_system_mesage()
            ...
            for value in message[1:]:
                for cnt in value:
                    if cnt[0] == "text":
                        ...
                        if  len(cnt[1]) >0:
                            for img in cnt[1]:
                                content = content.replace("<IMG_CONTEXT>", self.image_pattern(next(image_token_num_list)), 1)
                    elif cnt[0] == "video":
                        if len(cnt[1]) >0:
                            content = content.replace("<VIDEO_CONTEXT>", self.video_pattern(next(video_token_num_list)), 1)
                    else:
                        raise ValueError(f"Unknown value type: {value[0]}")
        ...
        input_video_mask = video_mask & input_mask
        output_video_mask = video_mask & ~input_mask

        tokenized_example["input_ids"][input_image_mask] = TYPE2INDEX["input"]["image"]
        # tokenized_example["input_ids"][output_image_mask] = self.type2index["output"]["image"]


        tokenized_example["input_ids"][input_video_mask] = TYPE2INDEX["input"]["video"]
        # tokenized_example["input_ids"][output_video_mask] = self.type2index["output"]["video"]

        # tokenized_example["labels"][output_image_mask] = IGNORE_INDEX  # the label will be filled in decoder.
        # debug_token_alignment(self.tokenizer, tokenized_example=tokenized_example)
        return tokenized_example
...
TEMPLATES = {
    ...
    "qwen2vl_intern": Qwen2VLChatTemplateWithInternData,
