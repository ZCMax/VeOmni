...
class InterleavedIterableDataset(IterativeDataset):
    def __init__(self, data: "HFIterableDataset",
            ...
            while True:
                try:
                    ...
                    if isinstance(transformed_sample, List):
                        for i in range(len(transformed_sample)):
                            transformed_sample[i]["ds_idx"] = ds_idx
                    ...
                    logger.warning(
                        f"[InterleavedIterableDataset] failed for sample with ds_idx={sample.get('ds_idx', None)}, "
                        f"error={repr(e)}, retry={retries + 1}/{self.max_retries}"
                    )

                    retries += 1
                    if retries >= self.max_retries:
                        logger.warning(
                            f"Exceeded max_retries={self.max_retries}. Skipping this sample."
                        )
                        break 
...
#             except Exception as e:
#                 logger.warning(f"[InterleavedMappingDataset] failed at index={index}, media at {sample['media_root']}"
#                                 f"error={repr(e)} (retry {retries + 1}/{self.max_retries})")

#                 retries += 1
#                 if retries >= self.max_retries:
#                     raise RuntimeError(
#                         f"Exceeded max_retries={self.max_retries} for index={index}"
#                     )
...
def build_interleave_dataset(
    ...
    **kwargs,
    ...
    logger.info_rank0(f"Start building interleave dataset with {len(sources)} subdataset.")
...
@DATASET_REGISTRY.register("internvl")
def build_interleave_dataset(
    train_path: str,
    datasets_type: str = "mapping",
    namespace: Literal["train", "test"] = "train",
    ...
    **kwargs,
    ...
    schedule = multisource_config["schedule"]
    source_names = multisource_config["names"]
    media_root = multisource_config["media_root"]
    ...
    if os.environ.get("DEBUG"):
        schedule = multisource_config["schedule"][:30]
        source_names = multisource_config["names"][:30]
        media_root = multisource_config["media_root"][:30]
        weights = schedule[0]["weights"][:30]
    ...
    if int(os.getenv("RANK", "0")) == 0:
        pbar = tqdm(total=len(sources), desc="Building datasets")
    if datasets_type == "iterable":
        logger.info_rank0("Start building iterable multisource dataset")

        def add_ds_idx_s3root_to_iterable(dataset, ds_idx, source_name, media_root):
            def gen():
                for x in dataset:
                    yield {**x, "ds_idx": ds_idx, "source_name": source_name, "media_root": media_root}

            return HFIterableDataset.from_generator(gen)

        for idx, source in enumerate(sources):
            if os.environ.get("DEBUG") and idx >=30:
                break
            dataset = build_iterable_dataset(source, namespace=namespace, seed=seed)
            ds = dataset._data
            ds = add_ds_idx_s3root_to_iterable(ds, idx, source_names[idx], media_root[idx])
            datasets.append(ds)
            if int(os.getenv("RANK", "0")) == 0:
                pbar.set_postfix({"id": idx, "name": source_names[idx]})
                pbar.update(1)
        ...
        for idx, source in enumerate(sources):
            ...
            ds = ds.add_column("ds_idx", [idx] * len(ds))
            ds = ds.add_column("source_name", [source_names[idx]] * len(ds))
            ds = ds.add_column("media_root", [media_root[idx]] * len(ds))
            datasets.append(ds)
        ...
        dataset = interleave_datasets(datasets=datasets, probabilities=weights, seed=seed)
        return InterleavedMappingDataset(
            dataset,
            transform=transform,
        )
